"""Enable APF on the proximity controller.

Calls /proximity_controller/enable with True, captures current position as anchor.
"""
...
class ApfOn(Node):
    def __init__(self):
        super().__init__('apf_on')
        self.client = self.create_client(
            SetBool, '/proximity_controller/enable'
        )

        self.get_logger().info('Enabling APF on proximity controller...')
    ...
    def _done_cb(self, future):
        try:
            ...
            self.get_logger().info(f'APF enabled: {result.message}')
